...
func TestServerRegistry_Phase2(t *testing.T) {
	...
	t.Run("GetOrLoadServer handles invalid config", func(t *testing.T) {
		...
		invalidConfig := &config.MCPClientConfigV2{
			TransportType: config.MCPClientTypeStdio,
			Env:           map[string]string{},
			Options:       &config.OptionsV2{},
		}
...
func TestResolveToolPath_Phase2(t *testing.T) {
	...
	t.Run("Resolve full tool path", func(t *testing.T) {
		...
		assert.Equal(t, config.MCPClientTypeStdio, serverConfig.TransportType)
...
func TestHandleExecuteTool_Phase2(t *testing.T) {
	...
	t.Run("Execute Serena tool through proxy", func(t *testing.T) {
		...
		// Log the actual content from Serena
		for i, content := range result.Content {
			if textContent, ok := mcp.AsTextContent(content); ok {
				t.Logf("Content[%d] (text):\n%s", i, textContent.Text)
			} else if imageContent, ok := mcp.AsImageContent(content); ok {
				t.Logf("Content[%d] (image): %d bytes", i, len(imageContent.Data))
			} else {
				t.Logf("Content[%d] (other type)", i)
			}
		}
...
func TestThreadSafety_Phase2(t *testing.T) {
	...
	t.Run("Concurrent GetOrLoadServer calls", func(t *testing.T) {
		...
		results := make(chan *client.Client, 10)
		errors := make(chan error, 10)

		for i := 0; i < 10; i++ {
			go func() {
				c, err := registry.GetOrLoadServer(ctx, "serena", cfg)
				if err != nil {
					errors <- err
				} else {
					results <- c
...
func TestMCPServerConfig_Phase2(t *testing.T) {
	t.Run("ToClientConfig converts MCPServerRef correctly", func(t *testing.T) {
		...
		assert.Equal(t, config.MCPClientTypeStdio, clientConfig.TransportType)
	...
	t.Run("ToClientConfig handles SSE type", func(t *testing.T) {
		...
		assert.Equal(t, config.MCPClientTypeSSE, clientConfig.TransportType)
