...
func TestRecursiveProxyBasics(t *testing.T) {
	...
	// Load hierarchy
	h, err := hierarchy.LoadHierarchy("../../testdata/mcp_hierarchy")
	...
	proxyClient, err := mcpclient.NewStreamableHttpClient(testServer.URL + "/")
...
// TestConfigLoading tests that the proxy config can be loaded from a file
func TestConfigLoading(t *testing.T) {
	cfg, err := config.Load("../../testdata/recursive_config_test.json", false, false, "", 0)
	require.NoError(t, err, "Failed to load config")

	assert.NotNil(t, cfg.McpProxy.Options)
	assert.NotNil(t, cfg.McpProxy)
