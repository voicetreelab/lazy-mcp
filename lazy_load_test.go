...
func TestLazyLoadingFlow(t *testing.T) {
	...
	mcpClient, err := newMCPClient("serena", config.McpServers["serena"])
	require.NoError(t, err, "Failed to create MCP client for serena")
	defer mcpClient.Close()

	server, err := newMCPServer("serena", config.McpProxy, config.McpServers["serena"])
...
func TestLazyLoadingPlaywright(t *testing.T) {
	...
	mcpClient, err := newMCPClient("playwright", config.McpServers["playwright"])
	require.NoError(t, err, "Failed to create MCP client for playwright")
	defer mcpClient.Close()

	server, err := newMCPServer("playwright", config.McpProxy, config.McpServers["playwright"])
