...
func TestResolveToolPath(t *testing.T) {
	...
	tests := []struct {
		...
		wantMapsTo     string
		wantServerNil  bool
		checkServer    func(t *testing.T, config *MCPClientConfigV2)
	}{
		{
		...
		},
		{
			...
			wantServerNil: false,
			checkServer: func(t *testing.T, config *MCPClientConfigV2) {
				assert.Equal(t, MCPClientTypeStdio, config.TransportType)
				assert.Equal(t, "uv", config.Command)
			},
		},
		{
...
func TestMCPServerRefToClientConfig(t *testing.T) {
	tests := []struct {
		name   string
		ref    *MCPServerRef
		check  func(t *testing.T, config *MCPClientConfigV2)
	}{
		{
			name: "stdio server",
			ref: &MCPServerRef{
		...
		},
		{
			...
			check: func(t *testing.T, config *MCPClientConfigV2) {
				assert.Equal(t, MCPClientTypeSSE, config.TransportType)
		...
		},
		{
			...
			check: func(t *testing.T, config *MCPClientConfigV2) {
				assert.Equal(t, MCPClientTypeStreamable, config.TransportType)
				assert.Equal(t, "http://localhost:9090", config.URL)
			},
		},
	}
